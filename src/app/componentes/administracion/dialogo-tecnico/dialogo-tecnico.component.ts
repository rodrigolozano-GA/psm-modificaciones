...
import { MatTableDataSource, MatSort, MatPaginator, MatDialogRef, MatDialog, MatSnackBar, MAT_DIALOG_DATA } from '@angular/material';
...
export class DialogoTecnicoComponent implements OnInit {
  ...
  estadoCuentasColumn: string[] = ['fecha', 'serie', 'folio', 'referencia', 'iddocumento', 'concepto', 'total'];  dataSource = new MatTableDataSource(this.inventarioList);
  ...
  constructor(
    ...
    if(this.data.hasOwnProperty("estatusName")){
      this.editar = this.data.estatusName == 'realizado' ? true : false;
    }
  }
