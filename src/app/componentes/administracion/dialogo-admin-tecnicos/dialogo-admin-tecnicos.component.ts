...
import { filter, takeUntil, tap, debounceTime, map, delay, finalize } from 'rxjs/operators';
...
/*
    ...
    if(this.generalForm.value.tecnico_principal != undefined) {
      //this.generalForm.get("tecnico_principal").patchValue({ tipo: 1 });
      let item = this.generalForm.value.tecnico_principal;
      item.tipo = 1;
      TecnicosPostData.push(item);
    }  */
    //this.overlayRef.detach();
    //console.log("TecnicosPostData: ", TecnicosPostData);
    TecnicosPostData.forEach(itemTecnico => {
      ...
      this._dataService.postData<any>('mesaControl/seguimiento/tecnicosFolio/save', "", detalleParams).pipe(
        ...
        dataS => { if(dataS["SUCCESS"]) { banTecnico = banTecnico + 1; varmsg = dataS["MESSAGE"]; } },
