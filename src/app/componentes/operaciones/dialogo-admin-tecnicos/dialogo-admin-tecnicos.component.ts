...
export class DialogoAdminTecnicosComponent implements OnInit {
    ...
    this._dataService.postData<any>(url, "", params)
      ...
      .subscribe(
        data => {
          ...
          this.tecnicosOPList = data.DATA; this.searching = false; 
          this.filteredTecnicosOPList.next(this.tecnicosOPList); //this.filtroTecnicos(); break;
          this.tecnicosOList = data.DATA; this.searching = false; 
          this.filteredTecnicosOList.next(this.tecnicosOList); //this.filtroTecnicos(); break;
          
          const listaT = this.dataTecnicos.data;
          ...
          for(let i = 0; i < data.DATA.length; i++)
          {
            for(let w = 0 ; w < this.parTec.length; w++)
            {
              if(data.DATA[i].nombre == this.parTec[w].nombre)
              {
                ...
                {
                  listaT.push(data.DATA[i]); // INSERTAR EN LA LISTA SOLO LOS QUE TENGAN ESTATUS == 0 
                }
               // listaT.push(data.DATA[i]);
              }
            }
          }
          this.dataTecnicos.data= listaT;
  ...
  selectTecnicos = () => {
    ...
    this.generalForm.patchValue({ tecnicos: this.dataTecnicos.data });
    //console.log("form:", this.generalForm.value.tecnico_principal);
  }
  ...
  removeTech(row: any)
  {
    const index = this.dataTecnicos.data.indexOf(row.id);
    this.dataTecnicos.data.splice(index, 1);
    this.dataTecnicos._updateChangeSubscription();
  ...
  ActionPost = () => {
    ...
    //Verifica que se ha agregado por lomenos el tecnico principal
    ...
    if(!this.dataTecnicos.data.length) 
    {
      return this._snack.open("No se han agregado técnicos", "", { duration: 2000, panelClass: ["snack-error"] });
    }
    ...
    if(this.generalForm.value.tecnico_principal != undefined) {
      let item = this.generalForm.value.tecnico_principal;
      item.tipo = 1;
      TecnicosPostData.push(item);
    }
*/
    ...
    //this.overlayRef.detach(); (myObject, index) =>
    TecnicosPostData.forEach((itemTecnico,index) => {
      let acc = 0;
      /*********     Guardar Técnicos     ****************/
       if(index == 0)
      {
      ...
      let detalleParams: any = {
        ...
        accion: acc//accion == 1 ? 1 : 0,
      };
      accion++;
      this._dataService.postData<any>('mesaControl/seguimiento/tecnicosFolio/save', "", detalleParams).pipe(
        finalize(() => {
          //console.log("accion: ", accion, " detalleParams: ", detalleParams);
          contTecnico++;
          if (contTecnico == this.generalForm.value.tecnicos.length) {
            this.overlayRef.detach();
            if (banTecnico != this.generalForm.value.tecnicos.length) {
              varmsg = "Algunos Técnicos no se guardaron correctamente";
            }
        ...
        data => { if(data["SUCCESS"]) { banTecnico = banTecnico + 1; varmsg = data["MESSAGE"]; } },
        error => { banTecnico = banTecnico; this.overlayRef.detach(); }
      );
      /***************************************************/
    });
  }
