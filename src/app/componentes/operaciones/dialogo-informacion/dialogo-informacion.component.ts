...
export class DialogoInformacionComponent implements OnInit {
  ...
  constructor(
    ...
    private _snack: MatSnackBar, private _overlay: Overlay) 
    {
      console.log("Data = " , data)
      this.accion = this.data.accion; // Si es nuevo  dta = 0
  ...
  dataChat = new MatTableDataSource(this.mensajesData);
  ...
  ngOnInit() {
    ...
    this.generalForm = this._fb.group({
      id: [this.servicio.id],
      fecha_programada: [{ value: this.servicio.fecha_programada, disabled: this.estatusName != 'en proceso' }],
    ...
    this.panelForm = this._fb.group({
      ...
      ot: [{ value: this.servicio.ot, disabled: this.estatusName != 'en proceso' }],
      tc: [{ value: this.servicio.tc, disabled: this.estatusName != 'en proceso' }]
    })
  ...
  dialogoTecnicos = (titulo?: string,item: any = null) => {
    console.log("Asignacion de tecnicos, parametros recibidos :   titulo : " , titulo, "item : ", item);
  ...
  addServicios = () => {
    if(this.estatusName != 'en proceso'){
      return this._snack.open("No se permiten realizar modificaciones con el estatus actual", "", { duration: 2000, panelClass: ["snack-error"] });
    }
    
    const _dialogRef = this._dialog.open(DialogoSeleccionServiciosComponent, {
  ...
  obtActualizaciones = (tabChangeEvent: MatTabChangeEvent, tipo = 0) => {
    //console.log("accion actual: ", this.accion);
  ...
  //Funcion para quitar Técnicos.
  removeTechnician= (row: any) =>
  {
    const dialogRef = this._dialog.open(DialogoConfirmacionesComponent, {
      panelClass: 'dialog-confirmaciones',
      data: { title: "Eliminar", text: `¿Está seguro de eliminar el técnico seleccionado?`, cancelAction: "NO", okAction: "SI" }
    })

    dialogRef.afterClosed().subscribe(res => {
      if (res) {
        this.confirmRemoveTechnician(res);
      }
    });
  }

  confirmRemoveTechnician = (reg: any = null) => {
    let params = { id: reg.id };
    this.mostrarCarga();
    this._dataService.postData<any[]>("mesaControl/seguimiento/tecnicoFolio/TechnicianDel", "", params).subscribe(
      data => {
        if(data["SUCCESS"]) {
          let params: any = { id: this.datosFolio.id };
          this.ObtenerDatos("mesaControl/seguimiento/serviciosFolio", params, 1, 1);
  ...
  ObtenerDatos = (url: string, params: any = null, tipo = 0, stopLoader = 0) => {
    this._dataService.postData<any[]>(url, "", params).subscribe(
      data => {
        switch(tipo) {
          case 1: this.dataSourceServ.data = data["DATA"];if(stopLoader) { this.overlayRef.detach(); } break; // Sericios de un folio
          ...
          case 7: this.overlayRef.detach();if(data["DATA"][0].tiempo.length == 0){data["DATA"][0].tiempo="0 MINUTOS";}this.dataActl.data = data["DATA"];break; // Lista de Actualizaciones de un Folio
...
/*
    ...
    this.dataSourceServ.data.forEach(function(x){
      //console.log('<->',x.nuevo,'<->');
    })
    */
    let params: any = { id: this.datosFolio.id };
        ...
        if(data["SUCCESS"]) {
          ...
          switch(tipo) {
            case 1: // Guardar Documento
              //console.log('case 1: Guardar documento');
              this.overlayRef.detach();
            ...
            case 2: // Cambiar Motivo y actualizar lista de técnicos
              //console.log('case 2 : Cambiar Motivo y actualizar lista de técnicos');
              this.overlayRef.detach();
              this.ObtenerDatos("mesaControl/seguimiento/tecnicosFolio", params, 2);
              this.ObtenerDatos("mesaControl/seguimiento/obtActividades", params, 8);
              break;
            ...
            let detalleParams: any = { folio_id: this.datosFolio.id, id: 0, codigo: 0, concepto: 0, precio: 0, nuevo : 0 };
          /*
            if(this.dataSourceServ.data.length){
              this._snack.open("Guardando y actualizando Servicios del Folio", "", { duration: 3000, panelClass: ["snack-ok"] });
            }
           */   
            /*Optimizando */
            let servicesSend: any = { folio_id:this.datosFolio.id, servicios:[]}

            this.dataSourceServ.data.forEach(itemServicio =>{
              if(!itemServicio.tipo)
              {
                let bannew = 0;
                if(itemServicio.nuevo == 1) { 
                  bannew = 1;
                }
                servicesSend.servicios.push(
                  {
                    "id": itemServicio.id_producto,
          ...
          this._dataService.postData<any>('operaciones/servicios/save', "", servicesSend)
          .subscribe(
            data => {
              this._snack.open("Datos actualizados", "", { duration: 3000, panelClass: ["snack-ok"] });
              this.overlayRef.detach();
              this._dialogRef.close();},
            error => { 
              this._snack.open("¡Ocurrio un error!", "", { duration: 3000, panelClass: ["snack-error"] });
              this.overlayRef.detach();}
          );
            /*end */
          ...
          /*
            this.dataSourceServ.data.forEach(itemServicio => {
              ...
              if(!itemServicio.tipo) {
                if(itemServicio.nuevo == 1) { detalleParams.nuevo = 1;}
               ...
               //this._dataService.postData<any>('mesaControl/servicios/save', "", detalleParams).pipe(
                this._dataService.postData<any>('operaciones/servicios/save', "", detalleParams).pipe(
                  finalize(() => {
                    countServicios++;
            ...
            break;*/
              break;
            case 4: 
            //console.log('case 4: reset overlay')
            this.overlayRef.detach(); this.actualizacionForm.reset(); this.generalForm.clearValidators(); this.obtActualizaciones(null, 1); break;
          }
        } else {
