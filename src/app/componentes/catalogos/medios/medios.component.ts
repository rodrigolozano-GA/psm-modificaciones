...
import { Medio } from '../../../clases/Catalogos/Medio';
...
export class MediosComponent implements OnInit {
  ...
  selection = new SelectionModel<Medio>(false, [])
  ...
  dialogoAgregar = () => {
    const _dialogRef = this._dialog.open(DialogoInformacionComponent, {
      ...
      data: { titulo: "Agregar medio", catalogo: "MD", accion: 0 } //0 => Nuevo | 1 => Editar
    });
  ...
  dialogEditar = (row) => {
    const _dialogRef = this._dialog.open(DialogoInformacionComponent, {
      panelClass: 'dialog-info',
      data: { titulo: "Editar medio", data: row, catalogo: "MD", accion: 1 }
    });
  ...
  exportAsExcel = () => {
    ...
    this.dataSource.filteredData.map(reg => {
      data.push({ "Medio": reg.nombre, "Estatus": reg.estatus ? 'Activo' : 'Inactivo' });
    })
  ...
  ActualizarTabla = () => {
    let data: any = {opc:1}
    this._dataService.postData<Medio[]>("catalogos/medios/all", "", data).subscribe(
      data => {
  ...
  cambiarEstatus = (registro: Medio) => {
      this._dataService.postData<any>("catalogos/medios/del", "", registro).subscribe(
        data => {
